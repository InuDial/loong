...
- `loong feishu send` and `loong feishu serve` remain available for Feishu-specific workflows, but the canonical cross-channel contract is `loong channels send feishu` and `loong channels serve feishu`.
