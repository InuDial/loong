...
| --- | --- | --- |
| direct send | `channels send feishu` | Usage: loong channels send feishu [OPTIONS] --target &lt;TARGET&gt; plus one of `--text`, `--post-json`, `--image-key`, `--image-path`, `--file-key`, or `--file-path` |
...
- `loong feishu send` and `loong feishu serve` remain available for Feishu-specific rich-send or family-specific workflows, but the canonical cross-channel contract is `loong channels send feishu` and `loong channels serve feishu`.
- grouped `channels send feishu` now accepts rich payload modes as part of the canonical contract, including `--post-json`, `--image-key` / `--image-path`, and `--file-key` / `--file-path` / `--file-type`.
